  ...
  function displayLT(jsonString){
   ...
   function displayTV(jsonString){
	...
	var flag = {};
	flag.id = (connection.id != undefined) ?  connection.id :  sessionStorage.getItem("sId");
	...
	console.log(flag)
	connection.emit('sendtoDevice',flag);
   ...
   function displayLV(jsonString){
	...
	var socket_id = localStorage.getItem("id" );
...
oldData = ""
  connection.on('comeFromDevice', function (data) {
	  call(data);
